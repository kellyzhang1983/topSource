    ...
    public Result addClientID(@RequestBody JSONObject body,HttpServletRequest request){
        ...
        try {
            ...
            if (ObjectUtils.isEmpty(clientId)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加clientId！");
            }

            Object password = body.get("password");
            if (ObjectUtils.isEmpty(password)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加password！");
            }

            Object clientName = body.get("clientName");
            if (ObjectUtils.isEmpty(clientName)){
                return new Result(false,StatusCode.SC_NOT_FOUND,"请添加clientName！");
            }

            Object authorizationGrantType = body.get("authorizationGrantTypes");
            if (ObjectUtils.isEmpty(authorizationGrantType)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加authorizationGrantTypes！");
            }

            Object redirectUris = body.get("redirectUris");
            if (ObjectUtils.isEmpty(redirectUris)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加redirectUris！");
            }

            Object logoutRedirectUris = body.get("logoutRedirectUris");
            if (ObjectUtils.isEmpty(logoutRedirectUris)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加logoutRedirectUris！");
            }

            Object scopes = body.get("scopes");
            if (ObjectUtils.isEmpty(scopes)){
                return new Result(false,StatusCode.SC_NOT_FOUND,"请添加scopes！");
            }

            Object tokenSettings = body.get("tokenSettings");
            if (ObjectUtils.isEmpty(tokenSettings)){
                return new Result(false,StatusCode.SC_NOT_FOUND,"请添加tokenSettings！");
            }
            ...
            if (ObjectUtils.isEmpty(accessTokenTimeToLive)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加accessTokenTimeToLive！");
            }
            ...
            if (ObjectUtils.isEmpty(refreshTokenTimeToLive)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加refreshTokenTimeToLive！");
            }
            ...
            if (ObjectUtils.isEmpty(authorizationCodeTimeToLive)){
                return new Result(false, StatusCode.SC_NOT_FOUND,"请添加authorizationCodeTimeToLive！");
            }
            ...
            log.error("添加客户端信息失败！请看详细信息：" + e.getMessage());
            return new Result(false, StatusCode.SC_INTERNAL_SERVER_ERROR,"添加客户端信息失败！请看详细信息",e.getMessage());
        }
        return new Result(true, StatusCode.SC_OK,"添加客户端信息成功！",jsonObject);
    }
}
