...
@Component
@Slf4j
public class AuthorizationMarketFilter extends OncePerRequestFilter {
    @Autowired
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(StringUtils.isEmpty(gatewayHeader)){
            if(StringUtils.isEmpty(GATWAY_TOKEN)){
                redisTemplate.boundValueOps(SystemConstants.redis_errorSecurityMarketService_message).set("请从网关进行访问!");
