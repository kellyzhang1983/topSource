...
public class OrderProcessListener implements RocketMQListener<Order>{
    ...
    public void onMessage(Order message) {
        String redisKey = "consumerGroup:" + RocketMQInfo.rocketMQ_consumer_orderProcess + "userId:" + message.getUserId() +   "msg:" + message.getId() ;
        try {
            if(redisTemplate.opsForValue().setIfAbsent(redisKey, "1", 24, TimeUnit.HOURS)){
                log.info("===========================================监听数据:<" + message.getId() + ">===============================================");
                orderService.createMarketOrder(message);
